...
def test_re_aiming_never_leaves_two_standing_programmes():
    ...
    firing, so the symptom is research that runs implausibly fast and an engine
    with no reason to suspect itself.

    This was real, in the player's own path: ``research --prefer industry``
    followed by ``research --prefer drive`` left two live orders, because the
    CLI called :func:`intents.research`, which always queues. Found by running
    the command twice against a real save.
    """
    ...
    with open_session(engine) as session:
        civ = civ_by_name(session, universe_id, "Terrans")
        assert session.get(Universe, universe_id) is not None
        for prefer in ("industry", "drive", None, "refining"):
            intents.steer_research(session, civ, prefer)
        session.flush()
        orders = [
            intent
        ...
        assert orders[0].payload.get("prefer") == "refining", (
            "the surviving order kept an old aim rather than the latest"
        )
