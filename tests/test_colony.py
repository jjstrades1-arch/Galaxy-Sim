...
from galaxysim.core.resources import METAL, VOLATILES
...
def _outpost(session, civ, *, habitability: float, stockpile: dict, world_type: str = "barren"):
    ...
    world.slots = 6
    world.resource_yield = {METAL: 1.0}
    colony = Colony(
        world_id=world.id,
        civ_id=civ.id,
        name="Outpost",
...
def test_labor_reallocation_shifts_output():
    ...
    with open_session(engine) as session:
        all_in_mining = session.get(Colony, colony_id).stockpile.get(METAL, 0.0)
        research_after_mining = civ_by_name(session, universe_id, "Terrans").research_points
    ...
    run_ticks(engine, universe_id, 24)
    with open_session(engine) as session:
        all_in_labs = session.get(Colony, colony_id).stockpile.get(METAL, 0.0)
        research_after_labs = civ_by_name(session, universe_id, "Terrans").research_points

    assert all_in_mining > all_in_labs
    assert research_after_labs > research_after_mining
...
def test_garden_world_needs_no_life_support():
    ...
    with open_session(engine) as session:
        civ = civ_by_name(session, universe_id, "Terrans")
        colony = _outpost(session, civ, habitability=1.0, stockpile={VOLATILES: 50.0})
        colony_id = colony.id
    ...
    with open_session(engine) as session:
        colony = session.get(Colony, colony_id)
        assert colony.stockpile[VOLATILES] == pytest.approx(50.0), "nothing should be burned"
...
def test_hostile_world_burns_supplies_then_starves_then_recovers():
    ...
    with open_session(engine) as session:
        civ = civ_by_name(session, universe_id, "Terrans")
        colony = _outpost(session, civ, habitability=0.0, stockpile={VOLATILES: 40.0})
        colony_id = colony.id
    ...
    with open_session(engine) as session:
        colony = session.get(Colony, colony_id)
        assert colony.stockpile[VOLATILES] < 40.0, "hostile worlds consume supplies"
    ...
    with open_session(engine) as session:
        colony = session.get(Colony, colony_id)
        assert colony.stockpile.get(VOLATILES, 0.0) == pytest.approx(0.0, abs=1e-6)
    ...
    with open_session(engine) as session:
        ...
        resupplied.stockpile[VOLATILES] = 500.0

    run_ticks(engine, universe_id, 48)
    with open_session(engine) as session:
        colony = session.get(Colony, colony_id)
        assert colony.population >= starving, "resupply must stop the dying"
        assert colony.stockpile[VOLATILES] < 500.0
...
def test_a_dome_makes_a_hostile_world_cheaper_to_hold():
    ...
    with open_session(engine) as session:
        civ = civ_by_name(session, universe_id, "Terrans")
        bare = _outpost(session, civ, habitability=0.0, stockpile={VOLATILES: 500.0})
        bare_id = bare.id

    run_ticks(engine, universe_id, 48)
    with open_session(engine) as session:
        burned_bare = 500.0 - session.get(Colony, bare_id).stockpile[VOLATILES]

    # Same colony, now domed.
    with open_session(engine) as session:
        colony = session.get(Colony, bare_id)
        colony.stockpile[VOLATILES] = 500.0
        session.add(
    ...
    run_ticks(engine, universe_id, 48)
    with open_session(engine) as session:
        burned_domed = 500.0 - session.get(Colony, bare_id).stockpile[VOLATILES]
...
def test_hydroponics_recycles_supplies():
    ...
    with open_session(engine) as session:
        civ = civ_by_name(session, universe_id, "Terrans")
        colony = _outpost(session, civ, habitability=0.0, stockpile={VOLATILES: 500.0})
        colony_id = colony.id

    run_ticks(engine, universe_id, 48)
    with open_session(engine) as session:
        burned_plain = 500.0 - session.get(Colony, colony_id).stockpile[VOLATILES]
    ...
    run_ticks(engine, universe_id, 48)
    with open_session(engine) as session:
        burned_recycled = 500.0 - session.get(Colony, colony_id).stockpile[VOLATILES]
...
def test_fleet_construction_requires_a_shipyard():
    ...
    with open_session(engine) as session:
        civ = civ_by_name(session, universe_id, "Terrans")
        colony = _outpost(session, civ, habitability=0.8, stockpile={METAL: 999.0, "energy": 999.0})
...
def test_buildings_need_industry_labor_to_finish():
    ...
    with open_session(engine) as session:
        civ = civ_by_name(session, universe_id, "Terrans")
        colony = home_colony(session, civ)
        colony.stockpile = {METAL: 999.0, "energy": 999.0, VOLATILES: 999.0}
...
def test_slots_limit_what_a_world_can_host():
    ...
    with open_session(engine) as session:
        civ = civ_by_name(session, universe_id, "Terrans")
        colony = home_colony(session, civ)
        colony.stockpile = {METAL: 9999.0, "energy": 9999.0, VOLATILES: 9999.0}
